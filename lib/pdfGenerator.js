...
class VoterPDFGenerator {
  ...
  static addVoterContent(doc, voter) {
    ...
    // Left column
    doc.text('Serial No (अनु क्रमांक):', leftX, startY, { continued: true }).font('Helvetica-Bold');
    doc.text(` ${voter.serialNumber || 'N/A'}`, { continued: false });

    doc.text('Voter ID:', leftX, doc.y, { continued: true }).font('Helvetica-Bold');
    doc.text(` ${voter.voterId || 'N/A'}`, { continued: false });
    ...
    // House Number
    doc.text('House No (घर क्र):', leftX, doc.y, { continued: true }).font('Helvetica-Bold');
    doc.text(` ${voter.houseNumber || 'N/A'}`, { continued: false }).font('Helvetica');

    // Booth & Ward
    doc.text('Ward (प्रभाग):', leftX, doc.y, { continued: true }).font('Helvetica-Bold');
    doc.text(` ${voter.actualWard || 'N/A'}`, { continued: false }).font('Helvetica');

    doc.text('Booth (बूथ):', leftX, doc.y, { continued: true }).font('Helvetica-Bold');
    doc.text(` ${voter.actualBooth || 'N/A'}`, { continued: false }).font('Helvetica');

    // Polling Center
    doc.moveDown(0.5);
    doc.text('Polling Center (मतदान केंद्र):', leftX, doc.y, { continued: true }).font('Helvetica-Bold');
    doc.font('Helvetica');
    const pollingCenter = voter.pollingCenter || 'N/A';
    doc.text(` ${pollingCenter}`, { width: 500 });
