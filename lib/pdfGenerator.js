...
class VoterPDFGenerator {
  ...
  static addVoterContent(doc, voter) {
    ...
    // Left column
    doc.text('अनु क्रमांक (Serial):', leftX, startY, { continued: true }).font('Helvetica-Bold');
    doc.text(` ${voter.serialNumber || 'N/A'}`, { continued: false }).font('Helvetica');

    doc.text('Voter ID:', leftX, doc.y, { continued: true }).font('Helvetica-Bold');
    doc.text(` ${voter.voterId || 'N/A'}`, { continued: false }).font('Helvetica');
    ...
    // House Number
    doc.text('खोली क्रमांक (House No):', leftX, doc.y, { continued: true }).font('Helvetica-Bold');
    ...
    const fullAddress = `मतदान केंद्र क्र. ${voter.actualBooth || voter.booth || '1'} – ${pollingCenter}`;
    const wrappedAddress = this.wrapText(fullAddress, 70);
    ...
    doc.text(` ${voter.actualBooth || voter.booth || 'N/A'}`, { continued: false }).font('Helvetica');

    doc.text('Ward:', leftX, doc.y, { continued: true }).font('Helvetica-Bold');
    doc.text(` ${voter.actualWard || voter.ward || 'N/A'}`, { continued: false }).font('Helvetica');
