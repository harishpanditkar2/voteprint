  ...
  static async extractVotersFromOCRText(text, pageNumber, pageImageBuffer, voterImagesDir, metadata) {
    ...
    if (pageImageBuffer && voterImagesDir) {
      try {
        ...
        // 3-column layout with proper margins and borders
        // Typical voter list has margins on all sides and borders between cards
        const leftMargin = Math.floor(imageWidth * 0.02); // 2% left margin
        const rightMargin = Math.floor(imageWidth * 0.02); // 2% right margin
        const topMargin = Math.floor(imageHeight * 0.05); // 5% top margin for header
        ...
        // Padding inside each cell to avoid borders (typically 1-2% of cell dimensions)
        const cellPaddingX = Math.floor(colWidth * 0.015); // 1.5% horizontal padding
        const cellPaddingY = Math.floor(rowHeight * 0.015); // 1.5% vertical padding
        ...
        for (let i = 0; i < voters.length; i++) {
          ...
          // Calculate position with margins and padding
          const left = leftMargin + (col * colWidth) + cellPaddingX;
          const top = topMargin + (row * rowHeight) + cellPaddingY;
          const width = colWidth - (2 * cellPaddingX);
          const height = rowHeight - (2 * cellPaddingY);
        ...
        console.log(`  📸 Saved ${voters.length} voter card images with border-aware cropping`);
      } catch (imgError) {
