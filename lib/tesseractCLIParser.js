...
class TesseractCLIParser {
  ...
  static async extractVotersFromOCRText(text, pageNumber, pageImageBuffer, voterImagesDir, metadata) {
    ...
    // Sort voters by serial number to match grid layout
    voters.sort((a, b) => {
    ...
    if (pageImageBuffer && voterImagesDir) {
      try {
        ...
        for (let i = 0; i < voters.length; i++) {
          ...
          const serialNum = parseInt(voter.serialNumber) || (i + 1);
          
          // Calculate grid position based on serial number
          // Serial numbers typically go left-to-right, top-to-bottom
          const voterIndex = (serialNum - 1) % 30; // Position within current page
          const row = Math.floor(voterIndex / 3);
          ...
          const cardImageFilename = `voter_${voter.voterId}_sn${voter.serialNumber}_page${pageNumber}.jpg`;
          ...
          if (i < 3) {
            console.log(`  📸 Voter #${voter.serialNumber}: row=${row}, col=${col}, pos=(${Math.floor(safeLeft)},${Math.floor(safeTop)}), size=${Math.floor(safeWidth)}x${Math.floor(safeHeight)}`);
          }
        }
        console.log(`  ✅ Saved ${voters.length} voter card images (sorted by serial number)`);
      } catch (imgError) {
