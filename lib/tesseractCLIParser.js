...
class TesseractCLIParser {
  ...
  static async extractPDFMetadata(pdfPath) {
    try {
      ...
      // Extract polling center address from header
      // Look for patterns like "मतदान केंद्र क्र. 3 - खोली क्र. 2, जिल्हा परिषद शाळा..."
      let pollingCenter = null;
      
      // Split text into lines for better processing
      const lines = ocrText.split('\n').map(line => line.trim()).filter(line => line.length > 10);
      
      // Look for polling center in first 10 lines (header area)
      for (let i = 0; i < Math.min(10, lines.length); i++) {
        const line = lines[i];
        
        // Pattern 1: "मतदान केंद्र क्र. X - address"
        if (line.includes('मतदान') && line.includes('केंद्र')) {
          // Take the full line as polling center if it has substantial content
          if (line.length > 20) {
            pollingCenter = line.replace(/\s+/g, ' ').trim();
            break;
          }
        }
        
        // Pattern 2: Check next line after "मतदान केंद्र"
        if (line.includes('मतदान केंद्र') && i + 1 < lines.length) {
          const nextLine = lines[i + 1];
          if (nextLine.length > 15) {
            pollingCenter = `${line} ${nextLine}`.replace(/\s+/g, ' ').trim();
            break;
          }
        }
      }
      ...
      if (!pollingCenter) {
        for (const line of lines.slice(0, 10)) {
          if ((line.includes('शाळा') || line.includes('सभागृह') || line.includes('भवन')) && line.length > 20) {
            pollingCenter = line.replace(/\s+/g, ' ').trim();
            break;
          }
        }
      ...
      console.log('📍 Extracted Polling Center:', pollingCenter || 'मतदान केंद्र');
      ...
      const filename = path.basename(pdfPath);
      const filenameMatch = filename.match(/Ward_(\d+)_Booth_(\d+)/i);
      console.log('⚠️ No metadata found in page 1, using fallback');
      return {
  ...
  static async extractVotersFromOCRText(text, pageNumber, pageImageBuffer, voterImagesDir, metadata) {
    ...
    // DON'T sort by serial number - OCR already extracts in visual grid order (left-to-right, top-to-bottom)
    // The serial numbers may not be sequential with visual layout
    // Keep voters in OCR extraction order for correct grid positioning
    ...
    if (pageImageBuffer && voterImagesDir) {
      try {
        ...
        // Maharashtra voter list layout analysis:
        // - Typically has 10 rows and 3 columns (30 voters per page)
        // - Has header row at top
        // - Borders between each cell
        // - Margins around the page
        
        // More precise margin calculations based on actual voter list format
        const leftMargin = Math.floor(imageWidth * 0.025); // 2.5% left margin
        const rightMargin = Math.floor(imageWidth * 0.025); // 2.5% right margin  
        const topMargin = Math.floor(imageHeight * 0.08); // 8% top margin (includes header)
        const bottomMargin = Math.floor(imageHeight * 0.02); // 2% bottom margin
        ...
        const rowsPerPage = 10; // 10 rows of voter cards
        const rowHeight = Math.floor(usableHeight / rowsPerPage);
        
        // Border thickness and padding - adjust these values to crop perfectly inside borders
        const borderThickness = Math.floor(colWidth * 0.008); // ~0.8% for border lines
        const innerPaddingX = Math.floor(colWidth * 0.02); // 2% horizontal padding inside cell
        const innerPaddingY = Math.floor(rowHeight * 0.02); // 2% vertical padding inside cell

        console.log(`  📏 Grid: ${colWidth}x${rowHeight} per cell, ${borderThickness}px borders`);

        // Group voters by page for correct position calculation
        const votersByPage = {};
        voters.forEach((voter, idx) => {
          const page = voter.pageNumber || 1;
          if (!votersByPage[page]) votersByPage[page] = [];
          votersByPage[page].push({ voter, originalIndex: idx });
        });

        // Process each page's voters separately
        Object.keys(votersByPage).forEach(page => {
          const pageVoters = votersByPage[page];
          
          pageVoters.forEach((item, positionInPage) => {
            const voter = item.voter;
            const i = item.originalIndex;
            
            // Position within THIS page (0-29 for a full page)
            const row = Math.floor(positionInPage / 3);
            const col = positionInPage % 3;
            
            // Calculate position: margin + (cell * index) + border/padding adjustments
            const left = leftMargin + (col * colWidth) + borderThickness + innerPaddingX;
            const top = topMargin + (row * rowHeight) + borderThickness + innerPaddingY;
            const width = colWidth - (2 * borderThickness) - (2 * innerPaddingX);
            const height = rowHeight - (2 * borderThickness) - (2 * innerPaddingY);
            
            // Ensure coordinates are within image bounds
            const safeLeft = Math.max(0, Math.min(left, imageWidth - 10));
            const safeTop = Math.max(0, Math.min(top, imageHeight - 10));
            const safeWidth = Math.max(10, Math.min(width, imageWidth - safeLeft));
            const safeHeight = Math.max(10, Math.min(height, imageHeight - safeTop));
            
            const cardImageFilename = `voter_${voter.voterId}_sn${voter.serialNumber}_page${page}.jpg`;
            const cardImagePath = path.join(voterImagesDir, cardImageFilename);
            
            // Only crop if we have the page image buffer (single page processing)
            if (voter.pageNumber === pageNumber) {
              sharp(pageImageBuffer)
                .extract({ 
                ...
                .catch(err => console.warn(`  ⚠️ Failed to crop ${voter.voterId}: ${err.message}`));
              
              voters[i].cardImage = `/voter-cards/${cardImageFilename}`;
              
              if (positionInPage < 3) {
                console.log(`  📸 #${voter.serialNumber} (${voter.name?.substring(0,15)}): pos=${positionInPage}, row=${row}, col=${col}, coords=(${Math.floor(safeLeft)},${Math.floor(safeTop)})`);
              }
            }
          });
        });
        
        console.log(`  ✅ Saved ${voters.length} voter card images (page-based positioning)`);
      } catch (imgError) {
